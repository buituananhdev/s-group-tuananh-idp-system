...
import { InternalServerErrorException } from 'src/common/exceptions/index';
...
export class Oauth2Service {
	...
	async login(LoginDto: LoginDto) {
		return await this.authenticationService.loginOAuth2(LoginDto);
	}

	async checkClient(client_id: string, redirect_uri: string) {
    console.log("Inside checkClient", client_id, redirect_uri);
		const client = await this.oauth2Repository.findOne({
